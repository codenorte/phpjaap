<html lang="es">
  <head>
    ...
    <title>JAAP | Factura Agua Sobrante</title>
  ...
  <body>
    ...
    <div class="cabecera">
      ...
      <div class="header_text2">
        @if($facturasobrante)
        <br>
        <h3>No. Comprobante: <strong>{{$facturasobrante->NUMFACTURA}}</strong> </h3>
        @endIf
      </div>
    ...
    <div class="datosUsuario">
      ...
      <div class="datosUsuario_text">
        <span>CAJA: {{$facturasobrante->USUARIOACTUAL}} </span> <br>
        <span>FECHA EMISION: {{$facturasobrante->FECHAEMISION}} </span>
      </div>
      <br>
      ...
      <div class="datosUsuario_text_left">
        <span>RUC/CI: {{$aguasobrante->users['RUCCI']}}</span><br>
        <span>CLIENTE: {{$aguasobrante->users['NOMBRES']}} {{$aguasobrante->users['APELLIDOS']}} </span><br>
        <span>SECTOR: {{$aguasobrante->users['SECTOR']}}</span>
      </div>
      @endIf
    </div>
          ...
          <tfoot>
            <tr>
              ...
              <td style="border-top: 1px solid black;text-align: right">
                <strong>{{ number_format($facturasobrante->SUBTOTAL, 2) }}</strong>
              </td>
            </tr>
            <tr>
              <td colspan = "1"></td>
              <td><strong>IVA:</strong></td>
              <td style="text-align: right;"><strong>{{ number_format($facturasobrante->IVA, 2) }}</strong></td>
            </tr>
            <tr>
              <td colspan = "1"></td>
              <td style="border-top: 1px solid black"><strong>TOTAL:</strong></td>
              <td style="border-top: 1px solid black;text-align: right;"><strong>{{ number_format($facturasobrante->TOTAL, 2) }}</strong></td>
            </tr>
         </tfoot>
        </table>
      </div>
